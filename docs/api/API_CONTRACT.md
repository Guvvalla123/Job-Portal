...
- **OpenAPI 3:** `docs/api/openapi.yaml` — partial snapshot (health, auth bootstrap, apply). Extend by exporting from Postman or editing the YAML.
...
Prefer a single source of truth over time: either maintain `docs/api/openapi.yaml` and generate clients, or export OpenAPI from Postman and replace the snapshot in CI when routes change.
